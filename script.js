// SALVAR campos no localStorage sempre que algo mudar
function salvarFicha() {
  ...
  campos.forEach(campo => {
    ...
    if (campo.type === "checkbox") {
      localStorage.setItem(id, campo.checked);
    } else if (campo.tagName === "SELECT" && campo.multiple) {
      const valores = Array.from(campo.selectedOptions).map(opt => opt.value);
      localStorage.setItem(id, JSON.stringify(valores));
    } else {
...
// RESTAURAR campos do localStorage ao carregar a página
function restaurarFicha() {
  ...
  campos.forEach(campo => {
    ...
    if (campo.type === "checkbox") {
      ...
      const valores = JSON.parse(valorSalvo);
...
// Listener para salvar automaticamente ao digitar ou mudar algo
window.addEventListener("DOMContentLoaded", () => {
  restaurarFicha(); // restaura quando a página abre
...
removeBtn.addEventListener('click', (e) => {
  e.stopPropagation(); // Impede de abrir o seletor de arquivos
  fileInput.value = '';
  previewImage.src = '';
